...
public class SQLOfertaAlojamiento {
	...
	public String consultarFuncionamiento(PersistenceManager pm) {
		Query q = pm.newQuery(SQL, "SELECT oc.semana, MAX(a.id_oferta) AS oferta_mas_ocupada, MIN(a.id_oferta) AS oferta_menos_ocupada, MAX(op.operador) AS operador_mas_solicitado, MIN(op.operador) AS operador_menos_solicitado FROM ( SELECT TO_CHAR(reserva.inicio, 'WW') AS semana, reserva.oferta, COUNT(*) AS cantidad_reservas FROM reserva GROUP BY TO_CHAR(reserva.inicio, 'WW'), reserva.oferta ) oc INNER JOIN ofertaalojamiento a ON oc.oferta = a.id_oferta INNER JOIN ( SELECT TO_CHAR(reserva.inicio, 'WW') AS semana, ofertaalojamiento.operador, COUNT(*) AS cantidad_reservas FROM reserva INNER JOIN ofertaalojamiento ON reserva.oferta = ofertaalojamiento.id_oferta GROUP BY TO_CHAR(reserva.inicio, 'WW'), ofertaalojamiento.operador )  op ON oc.semana = op.semana AND a.operador = op.operador GROUP BY oc.semana");
		List<Object[]> results = (List<Object[]>) q.execute();
		StringBuilder rta = new StringBuilder();
